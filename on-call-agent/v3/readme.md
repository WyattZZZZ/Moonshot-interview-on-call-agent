...
v1 和 v2 检索通过 `ThreadPoolExecutor(max_workers=2)` 并行执行。v3 在进程内导入 v1/v2 search 模块，不调用 v1/v2 HTTP API。融合时使用 v1/v2 返回的真实 0-1 分数，不再把每次检索结果强行归一化到最高分 1，因此无关 query 可以自然落到阈值以下。
...
如果 v1 或 v2 其中一个检索源不可用，v3 会记录该检索源的错误并继续使用另一个检索源的结果。错误会出现在 `retrieval.errors` 中，前端 trace 会显示“部分检索源不可用”。

...
由于项目使用本地 embedding 模型，初次给agent发送消息需要从 hf 上拉取模型文件，回复较慢，请见谅

同时，如果要初始化大量SOP文件的话，请将文件分别放置在`coding-exam/question-1/data`和`on-call-agent/data`中，数据库初始化从后者读取所有文件

## 一键启动
在根目录下

使用 `git submodule update --init --recursive` 填充依赖
...
`run_all.sh` / `run_all.bat` 会在启动任何服务和打开网页之前先运行 `init_data.py`。这个步骤会初始化 SQLite schema、导入 `../coding-exam/question-1/data/*.html` 到 v1 关键词索引，并为 v2 生成 Chroma 语义索引。如果 data 目录不存在或没有 HTML 文件，启动会直接失败，避免打开一个空数据库的前端。
...
v3 当前只保留综合评分 `>= 0.7` 的候选文档。这个阈值由代码常量统一控制，检索事件和错误文案都会返回同一个值。分数使用 v1/v2 返回的真实 0-1 分数，不再把每次检索结果强行归一化到最高分 1。如果没有任何候选文档达到阈值，v3 会继续请求 Kimi，但会在提示词中明确注入“未找到本地 SOP 文档”，并且不向模型暴露 `readFile` 工具。
